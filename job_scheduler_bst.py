...
class Node:
    def __init__(self, key):
        sched_time, duration, job_type = key.split(",")
        ...
        self.job_type = job_type.rstrip()
        self.left_child = None
        self.right_child = None

    def __str__(self):
        return f"Time: {self.data}, Duration: {self.duration}, End: {self.scheduled_end}, Jobname: {self.job_type}"
...
class BST:
    ...
    def helpful_print(self, key, succeeded):
        if succeeded:
            print(f"Added:\t\t {key.job_type}")
            print(f"Begin:\t\t {key.data}")
            print(f"End:\t\t {key.scheduled_end}")
            print("-"*60)
        else:
            print(f"Rejected:\t {key.job_type}")
            print(f"Begin:\t\t {key.data}")
            print(f"End:\t\t {key.scheduled_end}")
    ...
    def search_key(self, key):
        return self._search_key(self.root, key)

    def _search_key(self, curr, key):
        if curr:
            if key == curr.data:
                return curr
            elif key < curr.data:
                return self._search_key(curr.left_child, key)
            else:
                return self._search_key(curr.right_child, key)
        return
    ...
    def delete_key(self, key):
        self._delete_key(self.root, None, None, key)

    def _delete_key(self, curr, prev, is_left, key):
        if curr:
            if key == curr.data:
                if curr.left_child and curr.right_child:
                    min_child = self.min_right_subtree(curr.right_child)
                    curr.data = min_child.data
                    self._delete_key(curr.right_child, curr, False, min_child.data)
                ...
                self._delete_key(curr.left_child, curr, True, key)
            elif key > curr.data:
                self._delete_key(curr.right_child, curr, False, key)
        else:
